...
export class OfficialPriceRetriveService {
  ...
  async getMapping(
    ...
    const firstFiltered = await this.getFirstFiltering(
      companyId,
      productId,
      grammage,
    );
  ...
  private createGraph(secondFiltered: SecondFiltered[]) {
    ...
    for (let i = 0; i < secondFiltered.length - 1; i++) {
      for (let j = i + 1; j < secondFiltered.length; j++) {
        ...
        if (a.isParentOf(b))
          graph
            .get(a.getOfficialPriceConditionId())
            .push(b.getOfficialPriceConditionId());
      }
    }
