...
} from 'src/@shared/api/inhouse/official-price.response';
...
import {
  CreateOfficialPriceDto,
  OfficialPriceConditionIdDto,
  OfficialPriceListDto,
  OfficialPriceMappingDto,
  OfficialPriceUpdateDto,
} from './dto/official-price.request';
...
export class OfficialPriceController {
  ...
  async getList(
    ...
    const { officialPrices, total } =
      await this.officialPriceRetriveService.getList(
        req.user.companyId,
        dto.skip,
        dto.take,
      );
    ...
    return {
      items: officialPrices.map((op) => {
        const wholesale = op.officialPriceMap.find(
          (opm) => opm.officialPriceMapType === 'WHOLESALE',
        );
        const retail = op.officialPriceMap.find(
  ...
  async get(
    ...
    const officialPrice = await this.officialPriceRetriveService.get(
      req.user.companyId,
      dto.officialPriceConditionId,
    );

    const wholesale = officialPrice.officialPriceMap.find(
      (opm) => opm.officialPriceMapType === 'WHOLESALE',
    );
    const retail = officialPrice.officialPriceMap.find(
      (opm) => opm.officialPriceMapType === 'RETAIL',
    );
    ...
    return {
      ...
      sizeX: officialPrice.sizeX || null,
      sizeY: officialPrice.sizeY || null,
  ...
  @Post()
  @UseGuards(AuthGuard)
  async create(@Request() req: AuthType, @Body() dto: CreateOfficialPriceDto) {
  ...
  async delete(
    ...
    await this.officialPriceChangeService.delete(
      req.user.companyId,
      param.officialPriceConditionId,
    );
  }
